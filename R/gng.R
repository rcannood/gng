...
gng_r <- function(x, max_iter, epsilon_b, epsilon_n, age_max, max_nodes, lambda, alpha, beta, verbose, make_logs) {
  ...
  if (make_logs) {
    ...
    node_log <- dplyr::bind_rows(node_loglist)
    nodepos_log <- dplyr::bind_rows(nodepos_loglist)
...
#' @export
gng_project <- function(gng_out, verbose = T) {
  # Project the nodes to a 2D plane
  igr <- igraph::graph_from_edgelist(as.matrix(gng_out$edges), directed = F)
  ...
  if (make_projection) {
    rf <- randomForestSRC::rfsrc(Multivar(GNG_x, GNG_Y) ~ ., data.frame(na.omit(S), node_proj, check.names = F))
